...
var calculateAll = async(stockID,options ) => {
	...
	let storeString ={};
	// 抓取所有股票
	const data = await getTWStockList(stockID);
	// 遍例抓取的股票陣列
	/* promise 序列化函數 */ 
...
(async function main(){
	...
	program
		...
		.option('-l, --save-allData', '(all才有用)除存所以抓取資料')
		.option('-c, --count <value>', '(all才有用)遍例的有效上限(default:10)')
	...
	//TODO:判斷有stockID 那是不是有效的股票
	if(stockID != undefined && (await getTWStockList()).indexOf(parseInt(stockID))===-1){
